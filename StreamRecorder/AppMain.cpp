...
std::vector<ResearchModeSensorType> AppMain::kEnabledRMStreamTypes = { ResearchModeSensorType::IMU_ACCEL, IMU_GYRO, IMU_MAG };
	...
	m_qrCodeValue("")
{
	...
	if (AppMain::kEnabledRMStreamTypes.size() > 0)
	{
		...
		m_scenario->InitializeIMU();
		m_scenario->InitializeCameraReaders();
	}

	//regular stream types this works automatically
...
winrt::Windows::Foundation::IAsyncAction AppMain::StartRecordingAsync()
{
	...
	if (archiveSourceFolder)
	{
		...
		//if we are dealing with RM sensors, do the following
		if (m_scenario)
		{
			m_scenario->StartRecording(archiveSourceFolder, m_mixedReality.GetWorldCoordinateSystem());
		}
		//if we are dealing with normal cameras
